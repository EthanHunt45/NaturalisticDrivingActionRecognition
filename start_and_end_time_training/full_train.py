...
if __name__ == "__main__":
    ...
    train_recs, val_recs = train_test_split(all_records, test_size=0.2, random_state=42, stratify=labels)
    ...
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE_FULL, shuffle=True, num_workers=NUM_WORKERS_FULL, pin_memory=True)
    val_loader   = DataLoader(val_ds,   batch_size=BATCH_SIZE_FULL, shuffle=False, num_workers=NUM_WORKERS_FULL, pin_memory=True)
    ...
    best_val_acc = 0.0
    checkpoint_full = os.path.join(PROJECT_ROOT, "final_best_model_checkpoint_with_end_time.pth")
